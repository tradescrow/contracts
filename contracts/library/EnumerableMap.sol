...
library EnumerableMap {
    ...
    internal
    view
    returns (address[] memory keysOut, uint256[] memory valuesOut)
    {
        uint256 len = map._inner._entries.length;

        keysOut = new address[](len);
        valuesOut = new uint256[](len);
    ...
    unchecked {
        for (uint256 i; i < len; ++i) {
            ...
            valuesOut[i] = uint256(map._inner._entries[i]._value);
        }
    }
    }
    ...
    function values(
        AddressToUintMap storage map
    ) internal view returns (uint256[] memory valuesOut) {
        uint256 len = map._inner._entries.length;
